...
exports.run = function(client, message, args) {
  ...
  if (!value) {
    message.reply(
      "**İstiklal Marşını Dinlemek İçin `-istiklalmarşı çal` Yazmalısın**"
    );
  } //15 temmuz değil ! ( MİLLİ MAARŞIMIZI DİNLİYEN ASIL NE MUTLU TÜRKİYEM DİYEN KİŞİDİR )?
  if (!message.member.voiceChannel)
    return message.reply(
      "**İstiklal Marşını Dinlemek İçin Lütfen Sesli Kanala Gir**"
    );
  if (value === "çal") {
    if (
      message.member.voiceChannel.join().then(connection => {
        const stream = ytdl("https://www.youtube.com/watch?v=Y75Km7dlt94", {
          filter: "audioonly"
        });
    ...
    const turk = new Discord.RichEmbed()
      ...
      .setImage(
        "https://cdn.glitch.com/a0edfb5a-8170-4f31-9a22-d8e215a7fa03%2F1495639834_tumblr_oohvro8afj1vedtgco1_400.gif?v=1583058359108"
      );
